...
async function processFiles() {
  ...
  for (const file of mdFiles) {
    ...
    content = content.replace(/^---\n([\s\S]*?)\n---/, (match, fm) => {
      ...
      newFm = newFm.replace(
        /^title:\s*['"]?(.*?)(?:【#\d+】|第\d+回|Part\s*\d+|#\d+)['"]?\s*$/im,
        'title: "$1"',
      );
      // クリーンアップ
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        trimmed.includes("ここまで読んでくれてありがとう") ||
        trimmed.includes("読んでいただきありがとう")
      )
        continue;
      ...
      if (
        category !== "tourism" &&
        (trimmed.includes("喫煙") || trimmed.includes("タバコ") || trimmed.includes("たばこ"))
      ) {
        continue;
      }
