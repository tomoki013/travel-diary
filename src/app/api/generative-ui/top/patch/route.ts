...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { schema: buildFallbackSchema("更新UIの解析に失敗しました"), meta: { generatedAt: Date.now(), ttlSeconds: TTL_SECONDS } },
      { headers: NO_STORE }
    );
  }
  ...
  if (!validation.success) {
    return NextResponse.json(
      { schema: buildFallbackSchema("更新UIを安全に表示できませんでした"), meta: { generatedAt: Date.now(), ttlSeconds: TTL_SECONDS } },
      { headers: NO_STORE }
    );
  }
