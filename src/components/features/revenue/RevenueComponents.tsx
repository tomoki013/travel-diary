...
export const ComparisonTable = ({ items }: { items: CtaItem[] }) => {
  ...
  return (
    <div className="grid gap-3 md:grid-cols-2">
      {items.map((item) => (
        <article key={item.title} className="bg-card rounded-2xl border p-4">
          <p className="text-xs font-semibold text-teal-700">{item.serviceName}</p>
          <h4 className="mt-1 text-sm leading-snug font-bold">{item.title}</h4>

          <div className="bg-muted/40 mt-3 rounded-xl p-3">
            <p className="text-muted-foreground text-xs font-semibold">比較ポイント</p>
          ...
          <p className="text-muted-foreground mt-3 text-xs">
            このリンクは{" "}
            <span className="text-foreground font-semibold">{item.destinationLabel}</span>{" "}
            へ移動します。
          </p>
...
export const NextStepLinks = ({ posts }: { posts: PostMetadata[] }) =>
  posts.length === 0 ? null : (
    <section className="via-background mt-10 rounded-3xl border border-amber-200/70 bg-gradient-to-br from-amber-50/80 to-orange-50/70 p-6 dark:border-amber-900/70 dark:from-amber-950/30 dark:to-orange-950/20">
        ...
        {posts.map((post, index) => (
          <li key={post.slug}>
            <Link
              ...
              className="group bg-background flex items-center justify-between rounded-2xl border border-amber-200/70 px-4 py-3 transition hover:border-amber-300 hover:bg-amber-50/40 dark:border-amber-900"
            >
              <div className="flex items-start gap-3">
                ...
                <span className="text-sm leading-relaxed font-medium">{post.title}</span>
              </div>
