...
| `journey` | `string` | 任意 | `journey.ts` の ID | `series: travel-diary` または `category: itinerary` |
...
- `series: travel-diary` の場合のみ `journey` を使用可能。
...
- 画像関連の孤立行や画像キャプション
- `目次`、`すべて表示`、目次リスト
- 外部誘導、SNS フォロー推奨、関連記事誘導だけの文
- `こんにちは` `いかがでしたか？` のような定型挨拶

## 9. 最終チェック

- 検索ノイズとして `http` `目次` `画像` `ぜひ見て` などが残っていないか。
